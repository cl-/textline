...
function getNode_atLineEnd(aNodeOnTheTargetLine){
  ...
  console.debug("analyzed_textline_node::", analyzed_textline_node); //perma
...
function getString_atLineEnd(aNodeOnTheTargetLine){
  ...
  //console.debug: Trying out the various methods:
  var try_variousSplitJoinMethods_resultList = [
    "the_lineEnd_node, Before LineEnd::\n",
    // "Using node (split,slice,join)-->", atn.node.nodeValue.split('').slice(0,atn.numOffsetChars_toLineEnd).join('')+"\n",
    "Using node (oneString, substr)-->", atn.textNode.nodeValue.substr(0,atn.numOffsetChars_toLineEnd)//+"\n",  //#WRONG: atn.node.nodeValue.substr(0,atn.numOffsetChars_toLineEnd).join('') --> String does not have this Array function join().
    // "Using wordArr (tokens)--> ", atn.wordArr.slice(0,atn.sliceTillIdx).join(' ')
  ];
  console.debug(try_variousSplitJoinMethods_resultList.join(''));
  console.debug("the_lineEnd_node, After LineEnd (split,slice,join)-->", atn.textNode.nodeValue.split('').slice(atn.numOffsetChars_toLineEnd, atn.textNode.nodeValue.length).join(''));
  console.debug("\n\n");
...
function getNode_atLineEnd_afterStartNode(containerDOM, startNode, containerPos){ //the containerDOM gives the lineWidth and lineHeight
  ...
  // console.debug("startIdx:: idx of startNode in its parent::",startIdx);
  for (var i=startIdx-1; i<(startIdx+1)+1; ++i){
    var currNodeHW = getHeightWidth(containerDOM.childNodes[i]);
    // console.debug("atIdx::",i,":height::"+currNodeHW.height,"-->",containerDOM.childNodes[i]);    
  ...
  console.debug("estLineHeight::",estLineHeight);
  ...
  // console.debug("Find word at line end:");
  ...
  for (var i=startIdx; i<containerDOM.childNodes.length; ++i){ //Must use childNodes.length (not childElementCount) to include the #text nodes.
    // console.debug("childNode #::",i,':height::'+getHeightWidth(containerDOM.childNodes[i]).height,"-->", containerDOM.childNodes[i]);
    ...
    //WARNING: Do not use "top" or "bottom". They are relative within a line, and are undependable for determining line-end.
      ...
      // console.debug("containerWidth::",getPosition(containerDOM).width, "containerRight::",getPosition(containerDOM).right);
      // console.debug("currNodeRight, nextNodeRight, nextNodeWidth::",currNodePos.right, nextNodePos.right, nextNodePos.width);
      ...
      if ( currNodePos.right === containerPos.right && nextNodePos.left <= currNodePos.left ){ //Must be less-or-equal - CurrNode could have started at the start of line.
        the_lineEnd_node = containerDOM.childNodes[i];
        isMultiLine = false;
        console.debug("Exact right! Smaller left!");
        break;
      }
    }//end if-else
  }//end for-loop iterating on childNodes after the Anchor.
  console.debug("the_lineEnd_node::",the_lineEnd_node, "isMultiLine::", isMultiLine);
  ...
  console.debug("nodeName::", the_lineEnd_node.nodeName,  "isText::", the_lineEnd_node.nodeName == "#text");
  if(the_lineEnd_node.nodeName == "#text"){
    theTextNode = the_lineEnd_node;
  }
  else if(the_lineEnd_node.nodeName != "#text"){
    console.debug(the_lineEnd_node);
  ...
  for (var i=0; i<wordArr.length; ++i){
    // console.debug("wordArr::",wordArr); //debugging
    var subsetOfText = wordArr.slice(0,i+1).join(' ');
    // console.debug("subsetOfText::","sliceTil",i+1+"::", subsetOfText); //debugging
    ...
    if (subsetOfText_HW.height > estLineHeight*1.8){
      // console.debug("ACTIVATED subsetOfText-->", wordArr.slice(0,i+1).join(' ')); //debugging
      console.debug("DESIRED subsetOfText-->", wordArr.slice(0,i).join(' '));
      console.debug("Last word (at lineEnd)--> ", wordArr[i-1]  );
...
function loadDependency_all(){
  ...
  function createDepdLoader_immutable(dependencySrc, fn){
    return function(){
      console.debug("Loader loading: "+dependencySrc);
